...
import com.ltyc.sms.connect.cmpp.CMPPCodecChannelInitializer;
import com.ltyc.sms.connect.cmpp.CMPPEndpointEntity;
...
import com.ltyc.sms.session.AbstractSessionLoginManager;
 ...
 * @author guht
 * @version 1.0
 * @Description 处理客户端或者服务端登陆，密码校验。协议协商 建立连接前，不会启动消息重试和消息可靠性保证
 * @create 2020/2/6
 */
public class CmppSessionLoginManager extends AbstractSessionLoginManager {
    private static final Logger logger = LoggerFactory.getLogger(CmppSessionLoginManager.class);

    /**
     * 连接状态
     **/

    public CmppSessionLoginManager(EndpointEntity entity) {
        super(entity);
    }
